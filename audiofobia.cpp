...
class Grafo{
    public:
    list< pair<int,int> > * adya_list; // un puntero a un vector que almacena pares
    int vertices; // cant de servidores
    
    
    Grafo(int n){
	    vertices=n;
	    adya_list= new list< pair<int,int> >[n+1]; // n vectores, (1 para cada vertice, cada vector rep la lista de adyacencia,
...
int DFS_MaxEnElCamino(Grafo arbol, int v, int destino, bool visit[], int maximo);
...
int MaxEnElCamino(Grafo arbol, int inicio, int destino) {
    ...
    bool visit[n+1] = {false}; // Vector de nodos visitados
    int maximo_ini = -1;
...
int DFS_MaxEnElCamino(Grafo arbol, int inicio, int destino, bool visit[], int maximo){
	visit[inicio] = true;
	if (inicio == destino){
		return 0;
	}
